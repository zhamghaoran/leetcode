...
public:
    ...
    TreeNode *myBuildTree1(const vector<int> &preorder, vector<int> inorder, int preorder_left, int preorder_right,
                           int inorder_left, int inorder_right) {
        if (preorder_left > preorder_right) {
            return nullptr;
        }
        ...
        int treeSize = inorder_root - inorder_left;
        root->left = myBuildTree1(preorder, inorder, preorder_left + 1, preorder_left + treeSize, inorder_left,
                                  inorder_root - 1);

        root->right = myBuildTree1(preorder, inorder, preorder_left + treeSize + 1, preorder_right,
                                   inorder_root + 1, inorder_right);
        return root;
    }
